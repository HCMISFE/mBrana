...
import com.jsi.mbrana.Workflow.Adapter.Reports.ListOfFacilityWithIssueAdapter;
...
public class ListofFacilityWithIssueActivity extends AppCompatActivity implements IDataNotification {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_listof_facility_with_issue);
        setTitle("Facilities With Issue");
        getSupportActionBar().setHomeButtonEnabled(true);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        progress = new ProgressDialog(this);

        loadListOfFacilities();
    }

    @Override
    protected void onResume() {
        super.onResume();
        _mTracker.setScreenName("Reports: Facilities With Issue");
        _mTracker.send(new HitBuilders.ScreenViewBuilder().build());
    }

    private void loadListOfFacilities() {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_home) {
            Intent intent = new Intent(ListofFacilityWithIssueActivity.this, MainNavigationActivity.class);
            startActivity(intent);
            return true;
        } else if (id == android.R.id.home) {
            if (getIntent().getStringExtra("NavigationFrom") != null &&
                    getIntent().getStringExtra("NavigationFrom").equalsIgnoreCase("Report")) {
                Intent intent = new Intent(ListofFacilityWithIssueActivity.this, ReportListActivity.class);
                startActivity(intent);
                return true;
            } else if (getIntent().getStringExtra("NavigationFrom") != null &&
                    getIntent().getStringExtra("NavigationFrom").equalsIgnoreCase("Dashboard")) {
                Intent i = new Intent(ListofFacilityWithIssueActivity.this, MainNavigationActivity.class);
                startActivity(i);
                return true;
            }
        }
        ...
        if (requestCode == 2) {
            try {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    institution.setIssueDate(jo.getString("LastIssuedDate"));
                    institution.setQuantityIssued(jo.getInt("QuantityIssued"));

                    institutions.add(institution);
                }
                ListView institiutionList = (ListView) findViewById(R.id.dashboard_listofFacilityWithIssue);
                ListOfFacilityWithIssueAdapter adapter = new ListOfFacilityWithIssueAdapter(ListofFacilityWithIssueActivity.this, R.layout.layout_facilitieswithnoreorde, institutions);
